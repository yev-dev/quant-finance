...
from qf.core.data import get_yahoo_query_historical_data
...
def load_raw_data(ticker, start_date, end_date):
    ...
    # if filepath not in os.listdir(DATA_DIR):
    #     data = yq.download(ticker, start=start_date, end=end_date, interval='1d', auto_adjust=True)
    #     data.to_csv(filepath)

    crash_df = pd.read_csv(filepath)

    crash_df['RET'] = (crash_df['close'] / crash_df['close'].shift(1)) - 1
    crash_df['DIFF'] = crash_df['close'].diff()
    crash_df.index = crash_df.index
    crash_df['BIDLO'] = crash_df['low']
    crash_df['ASKHI'] = crash_df['high']
    crash_df['PRC'] = crash_df['close']
    crash_df['VOL'] = crash_df['volume']
    typical_price = (crash_df['high'] + crash_df['low'] + crash_df['close']) / 3
    crash_df['VWAP'] = (typical_price * crash_df['volume']).cumsum() / crash_df['volume'].cumsum()
